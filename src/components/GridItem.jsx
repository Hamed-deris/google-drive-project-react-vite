...
const GridItem = ({ file }) => {
  const { pushInserted, popInserted, setContextFile } = useStoreActions(
    (a) => a
  ); //action=>action
  ...
  const handleContext = (file) => {
    setContextFile(file);
  };
  return (
    <div
      onContextMenu={() => handleContext(file)}
      className="flex flex-col h-40 rounded-md items-baseline border border-blue-200  hover:bg-blue-50"
    >
